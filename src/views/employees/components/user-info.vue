  ...
  methods: {
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        this.$refs.staffPhoto.fileList = [{ url: this.userInfo.staffPhoto, uplaod: true }]
      }
    },
    async getPersonalDetail() {
      this.formData = await getPersonalDetail(this.userId)
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.mysStaffPhoto.fileList = [{ url: this.formData.staffPhoto, uplaod: true }]
      }
    },
    async savePersonal() {
      const fileList = this.$refs.mysStaffPhoto.fileList
      if (fileList.some(item => !item.uplaod)) {
        this.$message.warning('haiyouweishanchuan')
        return
      }
      await updatePersonal({ ...this.formData, staffPhoto: fileList.lenght ? fileList[0].url : ' ' })
    ...
    async saveUser() {
      ...
      await saveUserDetailById({ ...this.userInfo, staffPhoto: fileList.lenght ? fileList[0].url : ' ' })
